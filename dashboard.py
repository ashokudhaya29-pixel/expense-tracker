...
if selected_phone:
    expense_query = expense_query.eq("user_phone", selected_phone)

expenses_res = expense_query.execute()
...
if debts.empty:
    ...
    debts["remaining_amount"] = debts["remaining_amount"].astype(float)
    debts["total_amount"] = debts["total_amount"].astype(float)

    total_debt = debts["remaining_amount"].sum()
    closed_count = len(debts[debts["status"] == "closed"])
    ...
    st.dataframe(
        debts[["debt_name", "total_amount", "remaining_amount", "status"]],
        use_container_width=True
    )
...
if debt_payments.empty:
    ...
    st.dataframe(
        debt_payments[["payment_date", "amount", "notes", "cycle_month"]],
        use_container_width=True
    )